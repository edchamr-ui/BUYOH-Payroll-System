...
class PayrollSAPInput(db.Model):
    """Period-specific UK Statutory Adoption Pay input."""

    __tablename__ = "payroll_sap_inputs"
    ...
    adoption_pay_period_start = db.Column(db.Date, nullable=False)
    ...
    adoption_evidence_received = db.Column(
        db.Boolean, nullable=False, default=False, server_default="false"
    )
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "payroll_period_id",
            "employee_id",
            name="uq_payroll_sap_input_period_employee",
        ),
        db.CheckConstraint(
            "average_weekly_earnings >= 0",
            name="ck_payroll_sap_input_awe_non_negative",
        ),
        db.CheckConstraint(
            "paid_days >= 0",
            name="ck_payroll_sap_input_paid_days_non_negative",
        ),
        db.CheckConstraint(
            "salary_withheld >= 0",
            name="ck_payroll_sap_input_salary_withheld_non_negative",
        ),
    )
